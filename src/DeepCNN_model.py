...
weight_99_file = '99_weight_deep_cnn_val_0.926_loss_0.138.hdf5'
...
class DeepCNN:
    ...
    def predict_custom(self, image_path):
        ...
        # Grid search for best_value
        alphas = np.linspace(0.3, 1, 15)
        betas = np.linspace(0.45, 1, 12)
        for alpha in alphas:
            for beta in betas:
                for threshold in alphas:
                    if threshold < alpha + beta:
...
if __name__ == '__main__':
    ...
    network.predict(TEST_DIR + '7.jpg', window_size=99, threshold=0.8)
